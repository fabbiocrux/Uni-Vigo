...
Reduction <- 
  ...
  summarise(media = mean(Tensile.max)
            #sdt = sd(Tensile.max)
                     ) 
...
Reduction <- Reduction %>% mutate(reduction.intra.vir = (100 - max(Virgin)*100/Virgin ) )

Reduction <- Reduction %>% mutate(reduction.intra.rec = (100 - max(Recycled)*100/Recycled ) )

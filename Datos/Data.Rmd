...
#ggsave("../Figures/Phase-1-2-correction.jpg", width=10, height=6, dpi = "print")
```
...
#  stat_smooth(data = Fase.II %>% filter(ID > 65) , method = "lm", formula = y ~ x, se=FALSE) +
 ...
 #stat_regline_equation(label.y = c(3.9,3.5), label.x = c(40,40) , data = Fase.II, aes(x=ID, y=Load.max, label = paste(..eq.label.., ..adj.rr.label.., sep = '~\n~')),  formula = y ~ poly(x, 2), inherit.aes = TRUE ) +
  ...
  annotate("segment", x = 70, xend = 100, y = 3.5, yend = 3.5, colour = "blue", size=0.5, alpha=0.8, arrow= arrow(ends = "both", angle = 30, length = unit(.2,"cm"))) +
  # geom_vline(xintercept = 80) +
  annotate("text", x = c(50, 90), y = 3.6, label = c("Region A", "Region B"))+
  labs(title="", 
       subtitle="", 
       caption="",
       y="Maximum Load (kN)",
